...
CREATE TABLE dim_mc_estado (
    ...
    CONSTRAINT fk_mc_estado_regiao FOREIGN KEY (dim_mc_regiao_sk_regiao) REFERENCES dim_mc_regiao (sk_regiao)
);
...
CREATE TABLE dim_mc_sub_categoria_prod (
    ...
    qt_nota_sub_categoria_prod              INT,
    PRIMARY KEY (sk_sub_categoria_prod),
    CONSTRAINT fk_mc_subcateg_categoria FOREIGN KEY (dim_mc_categoria_prod_sk_categoria_prod) REFERENCES dim_mc_categoria_prod (sk_categoria_prod)
);
...
CREATE TABLE dim_mc_produto (
    ...
    CONSTRAINT fk_mc_produto_sub_categoria FOREIGN KEY (dim_mc_sub_categoria_prod_sk_sub_categoria_prod) REFERENCES dim_mc_sub_categoria_prod (sk_sub_categoria_prod)
);
...
CREATE TABLE dim_mc_cidade (
    ...
    CONSTRAINT fk_mc_cidade_estado FOREIGN KEY (dim_mc_estado_sk_estado) REFERENCES dim_mc_estado (sk_estado)
);
...
CREATE TABLE dim_mc_bairro (
    ...
    CONSTRAINT fk_mc_bairro_cidade FOREIGN KEY (dim_mc_cidade_sk_cidade) REFERENCES dim_mc_cidade (sk_cidade)
);
...
CREATE TABLE dim_mc_colecao_status (
    sk_colecao_status INT NOT NULL IDENTITY(1,1),
    sk_forma_pagto    INT NOT NULL,
    sk_origem_vda     INT NOT NULL,
    sk_status_venda   INT NOT NULL,
    sk_tipo_venda     INT NOT NULL,
    PRIMARY KEY (sk_colecao_status),
    CONSTRAINT fk_mc_colecao_status_forma_pagto FOREIGN KEY (sk_forma_pagto) REFERENCES dim_mc_forma_pagto (sk_forma_pagto),
    CONSTRAINT fk_mc_colecao_status_origem_vda FOREIGN KEY (sk_origem_vda) REFERENCES dim_mc_origem_vda (sk_origem_vda),
    CONSTRAINT fk_mc_colecao_status_status_venda FOREIGN KEY (sk_status_venda) REFERENCES dim_mc_status_venda (sk_status_venda),
    CONSTRAINT fk_mc_colecao_status_tipo_venda FOREIGN KEY (sk_tipo_venda) REFERENCES dim_mc_tipo_venda (sk_tipo_venda)
);
